...
:: 4. 预装需要编译的包（使用预编译wheel，避免需要C++编译器）
echo %yellow%安装预编译依赖（greenlet）...%reset%
pip install greenlet --only-binary :all: -i https://pypi.tuna.tsinghua.edu.cn/simple
if errorlevel 1 (
    echo %yellow%⚠ greenlet预编译安装失败，尝试指定版本...%reset%
    pip install greenlet==3.0.3 --only-binary :all: -i https://pypi.tuna.tsinghua.edu.cn/simple
)
