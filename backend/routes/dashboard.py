...
def get_category_stats():
    """
    分类统计（基于 labels JSON 字段提取分类信息）
    """
    try:
        conn = get_db_connection()
        cursor = conn.cursor()

        # 从 labels JSON 中提取分类信息
        cursor.execute("""
            SELECT labels FROM goods_list WHERE labels IS NOT NULL
        """)
        ...
        for row in rows:
            try:
                if row['labels']:
                    ...
                    # 找出 id='category' 的项
                    for label in labels:
                        if label.get('id') == 'category':
                            cat_name = label.get('name', '其他')
        ...
        # 转换为列表并排序
        result = [{'name': cat, 'value': count} for cat, count in category_count.items()]
